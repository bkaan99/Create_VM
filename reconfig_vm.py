...
def reconfigure_vm(vm, cpu_count, memory_mb, disk_size_gb):
    try:
        ...
        print("VM new Memory: %s" % memory_mb)
        print("VM new CPU: %s" % cpu_count)

        # Print information about each disk
        for device in vm.config.hardware.device:
            if isinstance(device, vim.vm.device.VirtualDisk):
                print(f"Disk {device.deviceInfo.label}: {device.capacityInKB / (1024 * 1024)} GB")
