...
for vm in vms:
    ...
    print(f"IP Adresi: {vm.summary.guest.ipAddress}")
    print(f"Güç Durumu: {vm.runtime.powerState == 'poweredOn' and 'Açık' or 'Kapalı'}")
    print(f"Oluşturulma Tarihi: {vm.summary.runtime.bootTime}")
    ...
    if (vm.snapshot is None):
        print("  Snapshot bulunmamaktadır.")
        print()
    else:
        for snapshot in vm.snapshot.rootSnapshotList:
            print(f"  Snapshot Adı: {snapshot.name}")
            print(f"  Snapshot Oluşturulma Tarihi: {snapshot.createTime}")
            print()
